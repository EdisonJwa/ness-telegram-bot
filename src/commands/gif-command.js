...
module.exports = (bot) => {
  // Question Command
  const rQuestion = new RegExp(`^/(gif|ㅇㅉ|움짤)(@${BOT_NAME})?$`, 'i')
    ...
    try {
      ...
      bot.onReplyToMessage(sent.chat.id, sent.message_id, async (message) => {
        ...
        if (!replyText) {
          ...
          googleImage.gif(replyText).then(async (results) => {
            ...
            const options = {
              reply_markup: JSON.stringify({
                inline_keyboard: [[
                  ...
                  { text: button.inline, switch_inline_query_current_chat: '/gif ' + replyText },
            ...
            try {
              ...
              sent = await bot.sendDocument(chatId, image.ou, options)
            } catch (err) {
              ...
              if (answer.data === 'more' && sent.message_id === answer.message.message_id) {
                if (queue.length === 0) {
                  ...
                  try {
                    ...
                    try {
                      bot.sendChatAction(chatId, 'upload_photo')
                      await bot.sendDocument(chatId, image.ou, options)
  ...
  // Query Command
  const rQuery = new RegExp(`^/(gif|ㅇㅉ|움짤)(@${BOT_NAME})?\\s+([\\s\\S]+)`, 'i')
  bot.onText(rQuery, (msg, match) => {
    ...
    googleImage.gif(text).then(async (results) => {
      const images = _.shuffle(results)
      const image = images[0]
      const options = {
        reply_markup: JSON.stringify({
